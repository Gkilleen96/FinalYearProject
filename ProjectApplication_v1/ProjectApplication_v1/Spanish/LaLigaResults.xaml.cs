...
namespace ProjectApplication_v1
{
	...
	public partial class LaLigaResults : ContentPage
	{
        apiData data = new apiData();
        string text = "";
        public LaLigaResults (apiData d1)
		{
            InitializeComponent();
            data = d1;
        ...
        public void checkDate(String date)
        {
            ...
            var jsonObject = JObject.Parse(data.espResults);


            int x = 27519;
            //fixture 1
            for (int i = 27139; i < 27519; i++)
            {
                DateTime event_date = (DateTime)jsonObject["api"]["fixtures"][i.ToString()]["event_date"];
                String time = event_date.ToString("hh:mm tt");
                ...
                if (dateCheck.Equals(date))
                {
                    ...
                    if (jsonObject["api"]["fixtures"][i.ToString()]["statusShort"].ToString().Equals("HT") || jsonObject["api"]["fixtures"][i.ToString()]["statusShort"].ToString().Equals("FT"))
                        TIME1.Text = jsonObject["api"]["fixtures"][i.ToString()]["statusShort"].ToString();
                    else if (jsonObject["api"]["fixtures"][i.ToString()]["statusShort"].ToString().Equals("NS"))
                        TIME1.Text = time;
                    else
                        TIME1.Text = jsonObject["api"]["fixtures"][i.ToString()]["elapsed"].ToString() + "\'";
